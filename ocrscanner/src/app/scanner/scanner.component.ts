...
export class ScannerComponent {
      ...
      reader.onload = () => {
        ...
        pdfjsLib.getDocument(pdfData).promise.then((pdf) => {
            ...
            pagePromises.push(
              pdf.getPage(i).then((page) => {
                return page.getTextContent().then((content) => {
                  ...
                  this.pdfPages[i - 1] = pageText; // Save the text of each page
                });
              })
            );
          }
          ...
          Promise.all(pagePromises).then(() => {
            this.extractedText = this.pdfPages[this.currentPages]; // Set text of the current page
          });
        });
      };
      reader.readAsArrayBuffer(file);
    ...
    extractTextFromImage(canvas: HTMLCanvasElement): void {
      Tesseract.recognize(
        ...
        this.extractedText = text;  // Update the extracted text
      }).catch((error) => {
